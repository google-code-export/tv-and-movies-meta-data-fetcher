...
/**
 * Used to parse and update MP4 Video files. This makes use of the jaudiotagger
 * library and extends it to support video files.
 */
public class MP4VideoFileReader {
	 ...
	 * and the encoding information present in the file. If the file has no tag,
	 * an empty one is returned. If the encoding information is not valid , an
	 * exception is thrown.
	 *
	 * @param f
	 *            The file to read
	 ...
	 *                Thrown if their is a tag Problem
	 * @exception ReadOnlyFileException
	 *                Thrown when accessing a read only file
	 * @exception InvalidAudioFrameException
	 *                Thrown if the a audio frame is invalid
	 * @exception CannotReadException
	 *                If anything went bad during the read of this file
	 */
	public AudioFile read(File f) throws CannotReadException, IOException,
		...
		try {
			raf = new RandomAccessFile(f, "r"); //$NON-NLS-1$
			raf.seek(0);
			...
			throw new CannotReadException(
					MessageFormat
							.format(Messages
									.getString("MP4VideoFileReader.UNABLE_READ_FILE"), f.getAbsolutePath()), e); //$NON-NLS-1$
		} finally {
			try {
				if (raf != null) {
					raf.close();
