...
{
    // Logger Object
    private static Log log = LogFactory.getLog(Mp4InfoReader.class);

    /**
     * Read the MP4 file header
     * @param raf The mp4 file
     * @return The mp4 File adder
     * @throws CannotReadException If anything went bad during the read of
	 *                this file
...
//        }
        ...
        if (info.getEncodingType().equals("")) //$NON-NLS-1$
        {
            info.setEncodingType(EncoderType.AAC.getDescription());
        }
        ...
        if (boxHeader != null)
        {
                    ...
                    || ftyp.getMajorBrand().equals(Mp4FtypBox.Brand.APPLE_AUDIO.getId()))
            {
                ...
                boxHeader = Mp4BoxHeader.seekWithinLevel(mvhdBuffer, Mp4AtomIdentifier.NMHD.getFieldName());
                if (boxHeader == null)
                {
                    throw new CannotReadVideoException(ErrorMessage.MP4_FILE_IS_VIDEO.getMsg());
                }
            ...
            {
                log.debug(ErrorMessage.MP4_FILE_IS_VIDEO.getMsg() + ":" + ftyp.getMajorBrand()); //$NON-NLS-1$
                throw new CannotReadVideoException(ErrorMessage.MP4_FILE_IS_VIDEO.getMsg());
            }
        }
